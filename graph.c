...
addressGraph AlokasiGraph(infoganteng X){
    addressGraph P;
...
void initGraph(Graph *G, int n){
    ...
    /* ALGORITMA */
    CreateEmptyGraph(G);
    FirstGraph(*G) = AlokasiGraph('B');
    P = FirstGraph(*G);
    NextGraph(P) = AlokasiGraph('S');
    for (i = 1; i <= n-2; i++) {
        NextGraph(P) = AlokasiGraph(i+'0');
        P = NextGraph(P);
    }
}
...
void AddLink(Graph *G, infoganteng n, infoganteng p){
    /* KAMUS LOKAL */
...
void PrintLink(Graph G, infoganteng X){
    /* KAMUS LOKAL */
    addressGraph P;
    ListL L;
...
boolean IsLinked(Graph G, infoganteng n, infoganteng p){
    /* KAMUS LOKAL */
    addressGraph addrGn;
...
addressGraph SearchGraph(Graph G, infoganteng X){
    /* KAMUS LOKAL */
    addressGraph addrG;
